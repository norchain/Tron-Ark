...
contract Team {
    ...
    //^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    RowingFun.Data private msData;
    function deleteAnyProposal(bytes32 _whatFunction) onlyDevs() public {RowingFun.deleteProposal(msData, _whatFunction);}
    function checkData(bytes32 _whatFunction) onlyAdmins() public view returns(bytes32 message_data, uint256 signature_count) {return(RowingFun.checkMsgData(msData, _whatFunction), RowingFun.checkCount(msData, _whatFunction));}
    function checkSignersByName(bytes32 _whatFunction, uint256 _signerA, uint256 _signerB, uint256 _signerC) onlyAdmins() public view returns(bytes32, bytes32, bytes32) {return(this.adminName(RowingFun.checkSigner(msData, _whatFunction, _signerA)), this.adminName(RowingFun.checkSigner(msData, _whatFunction, _signerB)), this.adminName(RowingFun.checkSigner(msData, _whatFunction, _signerC)));}
    ...
    */
    function addAdmin(address _who, bytes32 _name, bool _isDev)
        ...
        onlyDevs()
    {
        if (RowingFun.multiSig(msData, requiredDevSignatures_, "addAdmin") == true) 
        {
            RowingFun.deleteProposal(msData, "addAdmin");
    ...
    */
        ...
        onlyDevs()
    {
        ...
        // checks passed
        if (RowingFun.multiSig(msData, requiredDevSignatures_, "removeAdmin") == true) 
        {
            RowingFun.deleteProposal(msData, "removeAdmin");
    ...
    */
    ...
    {  
        ...
        if (RowingFun.multiSig(msData, requiredDevSignatures_, "changeRequiredSignatures") == true) 
        {
            RowingFun.deleteProposal(msData, "changeRequiredSignatures");
    ...
    */
    ...
    {  
        ...
        if (RowingFun.multiSig(msData, requiredDevSignatures_, "changeRequiredDevSignatures") == true) 
        {
            RowingFun.deleteProposal(msData, "changeRequiredDevSignatures");
